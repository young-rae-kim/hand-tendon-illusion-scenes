...
public class SkeletonMeshSlicer : MonoBehaviour
{
    ...
    public class SliceTarget
    {
        ...
        /// </summary>
        public SmMeshData _sMDataBase;
        public List<Transform> sectionsBones, posNoneSectionBones, negNoneSectionBones, posInvisibleBones, negInvisibleBones, allRootBones = new();
        /// <summary>
        ...
        {
            public List<SmMeshData> colMeshDatas = new();
        ...
        public SliceTarget(SkeletonMeshSlicer sMSlicer, SkinnedMeshRenderer sMRender, int i)
        {
            _sMSlicer = sMSlicer;
            _sMRender = sMRender;
            _sMDataBase = new SmMeshData(_sMRender.sharedMesh, _sMRender.rootBone.parent, _sMRender.sharedMaterials, _sMRender.bones); 
            targetIndex = i;
            ...
            foreach (var bone in sectionsBones)
            {
                ...
                {   
                    ...
                    foreach (var col in cols)
                    {
                        ...
                        var colMeshData = new SmMeshData(colMesh, bone);
        ...
        void DistinguishBothSideIndexes(SmMeshData nowDataBase)
        {
        ...
        void AddSectionTrianglesInMesh(List<int> nowSectionPoints, List<List<SectionTriangle>> nowDiffSectionTriangles, List<int> nowSideTriangles, SmMeshData nowDataBase)
        {
        ...
        public int AddNewSectionVertices(SmMeshData nowDataBase, int from, int to)
        {
        ...
        List<List<Vector2>> CreateAllCaps(List<List<int>> posOrderedCapEdges, List<List<int>> posCapsTriangles, List<List<int>> negCapsTriangles, SmMeshData nowDataBase)
        {
		...
		/// </summary>
		List<Vector2> ConvertModalV3ToPlaneV2(List<int> pointsIndexes, SmMeshData nowDataBase)
		{
        ...
        void DivideDifferentEdge(List<IndexVector> sectionEdges, List<List<int>> orderedCapEdges, SmMeshData nowDataBase)
        {
        ...
        void CalculateCapNormalsAndSwitchCapSide(List<List<int>> posCapsTriangles, List<List<int>> negCapsTriangles, List<Vector3> posCapNormals, List<Vector3> negCapNormals, List<List<int>> posOrderedCapEdges, List<List<int>> negOrderedCapEdges, SmMeshData nowDataBase)
        {
    ...
    public class SliceableTriangle
    {
		...
		/// </summary>
		public void DivideByPlane(SmMeshData nowDataBase, int subMeshIndex, bool _side1, bool _side2, bool _side3)
		{
		...
		/// </summary>
		static void CalculateWithOneNegTriangle(int neg, int posA, int posB, SmMeshData nowDataBase, int subMeshIndex)
		{
			//            pos
			//  posA\.^^^^^^^^^^^^/posB
			//       \   .       /
		...
		/// </summary>
		static void CalculateWithTwoNegTriangle(int pos, int negA, int negB, SmMeshData nowDataBase, int subMeshIndex)
		{
			//            pos
			//            / \
			//           /   \
    ...
    void Start() 
    {
        ...
        sMRenders = GetComponents<SkinnedMeshRenderer>();
        sliceTargets = new SliceTarget[sMRenders.Length];
        isInCut = new bool[sMRenders.Length];
        for (int i = 0; i < sMRenders.Length; i++) 
        {
            Debug.Log(sMRenders[i].sharedMesh);
            Debug.Log(sMRenders[i].rootBone.parent);
            Debug.Log(sMRenders[i].sharedMaterials);
            Debug.Log(sMRenders[i].bones);
